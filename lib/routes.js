...
router.get('/',
  async (req, res) => {
    ...
    if (req.headers.accept === 'application/json') {
      res.send({ recaptchaSiteKey, requestVerificationToken, cookieRVValue, aspSessionId })
    } else {
      res.render('index', { recaptchaSiteKey, requestVerificationToken, cookieRVValue, aspSessionId })
    }
  },
)
